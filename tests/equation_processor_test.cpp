...
public:
    MOCK_METHOD(std::shared_ptr<calc::EquationSolver>, create_equation_solver, (const long double&), (override));
};
...
TEST(EquationProcessorTestSuite, ShouldProcessEquationInputAndPrintSolution) {
    ...
    EXPECT_CALL(*equation_solver_factory, create_equation_solver(-143.0))
