...
namespace mapper {
    ...
    NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(Response, initialEquation, resultEquation, solution)
}
...
mapper::Response serialize_under_damped(std::shared_ptr<calc::Solution> solution_ptr) {
    ...
    mapper::Response response{solution_ptr->get_initial_equation(), solution_ptr->get_result_equation(), solution};
    return response;
}
